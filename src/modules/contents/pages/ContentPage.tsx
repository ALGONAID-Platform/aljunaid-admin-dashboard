...
import { 
  ...
  RefreshCw, Settings2, FolderOpen, Search
} from 'lucide-react';
...
import { MediaIndicator } from '../../../components/ui/MediaIndicator';
...
export function ContentPage() {
  ...
  useEffect(() => { setCurrentPage(1); }, [searchQuery, activeView, itemsPerPage]);
  ...
  const filtered = content.filter(c => {
    const matchType = activeView === 'all' || (activeView === 'video' && c.type === 'video') || (activeView === 'markdown' && c.type === 'markdown');
    const matchSearch = c.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
                        (c.lessonTitle && c.lessonTitle.toLowerCase().includes(searchQuery.toLowerCase()));
    let matchesExtra = true;
    if (activeView === 'no_pdf') matchesExtra = !(c as any).pdfUrl;

    return matchType && matchSearch && matchesExtra;
  });
    ...
    <div className="font-sans antialiased text-slate-800 space-y-6" style={{ fontFamily: "'Cairo', sans-serif" }}>
      ...
      <StickyToolbar position="top">
        <GlobalSearch 
          ...
          placeholder="البحث في المحتوى أو الدروس..."
        />
        <SavedViews 
          ...
          views={[
            ...
            { id: 'video', label: 'مقاطع الفيديو', icon: Video },
            { id: 'markdown', label: 'المحتوى النصي', icon: FileText },
            { id: 'no_pdf', label: 'بدون ملفات PDF', icon: FileType2 },
          ]}
        />
      </StickyToolbar>
