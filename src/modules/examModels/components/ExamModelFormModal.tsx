...
import {
  X, Plus, Edit3, Loader2, AlertCircle, FileText, Upload, Link as LinkIcon, CheckCircle
} from 'lucide-react';
...
export function ExamModelFormModal({
  ...
  // جلب المقررات مرة واحدة عند الحاجة
  useEffect(() => {
    if (courses.length === 0) {
      void fetchCourses();
    }
  ...
  // 1. تهيئة الفورم عند الفتح أو التعديل (معزولة تماماً عن تغييرات المقررات)
  useEffect(() => {
    if (isOpen) {
      if (editingModel) {
        setForm({
  ...
  // 2. تعيين المقرر الافتراضي بأمان دون التأثير على الفورم
  useEffect(() => {
    if (isOpen && !editingModel && !form.courseId && courses.length > 0) {
      setForm((prev) => ({ ...prev, courseId: String(courses[0].id) }));
    }
  }, [isOpen, courses, editingModel, form.courseId]);

  const handlePdfFileSelect = (file: File) => {
    // دعم إضافي لأنظمة التشغيل التي لا ترسل MIME type بشكل صحيح
  ...
  const validate = (): boolean => {
    // إزالة تحويل الرقم، وفحص الفورم مباشرة كما هو مصمم في Zod
    ...
    if (!result.success) {
      ...
      result.error.issues.forEach((issue) => {
        ...
        if (!formattedErrors[pathKey]) {
          ...
          allErrors += ` - ${issue.message} `;
        }
      });

      // إجبار الفورم على إظهار الأخطاء المخفية
      formattedErrors.submit = `تعذر الحفظ بسبب:${allErrors}`;
      setErrors(formattedErrors);
      return false;
    }

    // التحقق اليدوي من وجود ملف أو رابط
    if (!pdfFile && !form.pdfUrl) {
      setErrors((prev) => ({ ...prev, submit: 'يرجى إرفاق ملف PDF أو إدخال رابط صالح للنموذج.' }));
      return false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validate()) {
      setErrors((prev) => ({
        ...prev,
        submit: prev.submit || 'يرجى مراجعة الحقول المطلوبة وتصحيح الأخطاء قبل الحفظ.',
      }));
      return;
    }
    ...
    try {
      const payload: CreateExamModelPayload & { id?: string } = {
        ...
        // إفراغ الرابط المؤقت إذا كان هناك ملف حقيقي ليتم رفعه
        pdfUrl: pdfFile ? '' : form.pdfUrl,
        pdfFile: pdfFile,
        grade: form.grade,
      };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-6 bg-slate-950/80 backdrop-blur-md transition-all">
      ...
      >
        ...
        {/* Form Content - noValidate لمنع تدخل المتصفح */}
            ...
            <div className="p-6 rounded-3xl bg-slate-50 border border-slate-100 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-3 bg-white border rounded-xl text-sm font-semibold outline-none transition-all ${errors.title ? 'border-red-300 bg-red-50/50' : 'border-slate-200 focus:border-emerald-500'
                    }`}
                />
            ...
            {pdfFile ? (
              <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-2xl flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="block text-sm font-bold text-emerald-800 line-clamp-1 truncate max-w-[200px]" dir="ltr">
                      {pdfFile.name}
                    </span>
                    <span className="block text-xs font-medium text-emerald-600 mt-0.5">
                ...
                <button
                  ...
                  className="w-8 h-8 flex items-center justify-center text-red-500 hover:text-red-700 bg-white hover:bg-red-50 rounded-lg border border-red-100 transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
            ...
            <div>
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pr-10 pl-4 py-3 bg-white border rounded-xl text-sm font-semibold outline-none transition-all text-left dir-ltr ${errors.pdfUrl ? 'border-red-300 bg-red-50/50' : 'border-slate-200 focus:border-emerald-500'
                    }`}
                />
              </div>
